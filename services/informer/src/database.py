"""
database.py — PostgreSQL connection pool for the Informer service.
...
# ─── Module-level lazy singleton (for callers that use the old get_pool API) ───

_db_pool: DatabasePool | None = None


def init_db_pool(settings) -> DatabasePool:
    """Create and initialise the global DatabasePool singleton."""
    global _db_pool
...
def get_db_pool() -> DatabasePool:
    """Return the global pool; raises if not yet initialised."""
    if _db_pool is None:
        raise RuntimeError("DatabasePool not initialised — call init_db_pool() first")
    return _db_pool


@contextmanager
def get_connection():
    """Convenience wrapper around the global pool's context manager."""
    yield from get_db_pool().get_connection()
